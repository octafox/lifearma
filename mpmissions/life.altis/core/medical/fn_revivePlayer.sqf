...
if (_target getVariable ["Reviving", objNull] isEqualTo player) exitWith {[localize "STR_Medic_AlreadyReviving","info",30] call life_fnc_notification_system;};
...
if !(_target getVariable ["Reviving", objNull] isEqualTo player) exitWith {[localize "STR_Medic_AlreadyReviving","info",30] call life_fnc_notification_system; life_action_inUse = false;};
_target setVariable ["Reviving", nil, true];

if (!alive player || {life_istazed} || {life_isknocked}) exitWith {life_action_inUse = false;};
if (_target getVariable ["Revive", false]) exitWith {[localize "STR_Medic_RevivedRespawned","info",30] call life_fnc_notification_system; life_action_inUse = false;};
